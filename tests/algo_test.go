...
func TestBinTree(t *testing.T) {
	...
	algo.InsertNode(&treeNode, algo.IntegerKey(4))
	algo.InsertNode(&treeNode, algo.IntegerKey(9))
	algo.InsertNode(&treeNode, algo.IntegerKey(8))
	algo.InsertNode(&treeNode, algo.IntegerKey(7))
	algo.InsertNode(&treeNode, algo.IntegerKey(11))
	avlTree, _ = json.MarshalIndent(treeNode, "", " ")
	t.Log(string(avlTree))
	t.Log("\n Delete Tree")
	algo.RemoveNode(&treeNode, algo.IntegerKey(4))
	algo.RemoveNode(&treeNode, algo.IntegerKey(8))
	avlTree, _ = json.MarshalIndent(treeNode, "", " ")
	t.Log(string(avlTree))
}
